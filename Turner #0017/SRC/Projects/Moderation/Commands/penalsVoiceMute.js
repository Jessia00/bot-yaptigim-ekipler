...
module.exports.execute = async(ModerationClient , message, args) => {
  ...
  if (!member) return moderationfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}voicemute @Richârd/ID Sebep Süre\``).sil(10000);
  if (!sebep) return moderationfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}voicemute @Richârd/ID Sebep Süre\``).sil(10000);
  if (!zaman) return moderationfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}voicemute @Richârd/ID Sebep Süre\``).sil(10000);
...
//////////
    ...
    let zamanimiz = zamanimizlar.replace("second"," saniye").replace("minute"," dakika").replace("hours"," saat").replace("day"," gün")
      ...
      moderationfxnfx.embedBasari(message.author, message.channel, `${member} kullanıcısı sesli kanallarda **${zamanimiz}** susturuldu (\`#${cezaID}\`)`)
    
//////////
moderationfxnfx.cezaEkle(cezaID, member.id, member.displayName, message.member.displayName, message.author.id, "VOICE-MUTE", sebep, zamanimiz, muteAtılma, muteBitiş, "voicemute")
let ceza = {  
...
member.roles.add(config.voicemuted)
...
setTimeout(() => {
    ...
    member.roles.remove(config.voicemuted)
