...
module.exports.execute = async(ModerationClient , message, args) => {
  ...
  if (!member) return moderationfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}chatmute @Richârd/ID Sebep Süre\``).sil(10000);
  if (!sebep) return moderationfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}chatmute @Richârd/ID Sebep Süre\``).sil(10000);
  if (!zaman) return moderationfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}chatmute @Richârd/ID Sebep Süre\``).sil(10000);
