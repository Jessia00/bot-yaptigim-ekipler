...
module.exports.execute = async(ModerationClient , message, args) => {
  ...
  if (!member) return moderationfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}voiceunmute @Richârd/ID Sebep\``).sil(10000);
  if (!sebep) return moderationfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}voiceunmute @Richârd/ID Sebep\``).sil(10000);
