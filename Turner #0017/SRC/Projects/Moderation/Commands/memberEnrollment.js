...
module.exports.execute = async(ModerationClient , message, args) => {
 let embed = new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setFooter(config.footer).setColor("RANDOM");
    let member = message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.member;
      ...
      let sicilsayi = penals.cek(`sicilsayi.${member.id}.${settings.server}`);
   let soncezainfo = penals.cek(`sonceza.${member.id}.${settings.server}`);
        let sicilminerdelanit = penals.cek(`sicil.${member.id}.${settings.server}`);
     if (!sicilminerdelanit) return Functions.embedHata(message.author, message.channel, `**${message.guild.name}** sunucusunda kayıtlı ${member} kullanıcısının sicil kayıdı bulunmuyor.`)

    let sicilinfo = penals.cek(`sicil.${member.id}.${settings.server}`);
...
${sicilimm}`,
                   ...
                   {name: settings.footer, icon: false},                 
