...
module.exports.execute = async(RegisterClient, message, args) => {
    ...
    if (!member) return registerfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}isim @Richârd/ID <İsim> <Yaş>\``).sil(10000);
    let yazilacakisim;
    let isim = args[1];
    ...
    if (!Number(args[2])) return registerfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}isim <@Richârd/ID> İsim Yaş\``).sil(10000);
    if (!member || !isim) return registerfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}isim <@Richârd/ID> İsim Yaş\``).sil(10000);
    ...
    registerfx.kayitEt(member, message.author, "isim", isim, yaş)
