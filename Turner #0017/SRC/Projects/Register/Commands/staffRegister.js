...
module.exports.execute = async(RegisterClient, message, args) => {
    ...
    if (!member) return registerfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}kayit @Richârd/ID <İsim> <Yaş>\``).sil(10000);
    let yazilacakisim;
    let isim = args[1];
    ...
    if(!member.user.username.includes(settings.tag) && !member.discriminator.includes(0017) && !member.roles.cache.has(registerConfig.vip) && !member.roles.cache.has(registerConfig.booster)) return message.react(emojis.hata);
    if (!Number(args[2])) return registerfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}kayıt <@Richârd/ID> İsim Yaş\``).sil(10000);
    if (!member || !isim) return registerfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}kayıt <@Richârd/ID> İsim Yaş\``).sil(10000);
    ...
    yazilacakisim = `${member.user.username.includes(settings.tag) ? settings.tag : (settings.untag ? settings.untag : (settings.tag || ""))} ${isim} | ${yaş}`;
