...
module.exports.execute = async(ModerationClient , message, args) => {    
      ...
      let etikettagli = message.guild.members.cache.filter(uye => uye.user.discriminator.includes("1856")).size;
    const voiceChannels = message.guild.channels.cache.filter(c => c.type === 'voice');
      let ses = 0;
      for (const [id, voiceChannel] of voiceChannels) ses += voiceChannel.members.size;
        let sestag = message.guild.members.cache.filter(uye => uye.user.username.includes(settings.nicktag) && uye.user.discriminator.includes(1856) && s.voice.channel).size
