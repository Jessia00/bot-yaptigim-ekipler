...
//////////
  ...
  if (!richardinuyesi || !sebep || !zaman) return moderationFunctions.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${config.prefix || '.'}mute @Richârd/ID (1s/1m/1h/1d) sebep\``).then(x => x.delete({timeout: 10000}));
 


if(message.member.roles.highest.position <= richardinuyesi.roles.highest.position) {
    return moderationFunctions.embedHata(message.author, message.channel, `Kendinden üst yetkide birini susturamazsın!`).then(x => x.delete({timeout:6500}));
}

//////////

if(!richardinuyesi.bannable) {
    return moderationFunctions.embedHata(message.author, message.channel, `${richardinuyesi} kişisini susturacak yetkim bulunmuyor.`).then(x => x.delete({timeout:6500}));
}
...
//////////
    ...
    let zamanimiz = zamanimizlar.replace("second"," saniye").replace("minute"," dakika").replace("hours"," saat").replace("day"," gün")
      ...
      moderationFunctions.embedBasari(message.author, message.channel, `${richardinuyesi} kullanıcısı **${zamanimiz}** cezalıya atıldı (\`#${cezaID}\`)`)
    
//////////
moderationFunctions.cezaEkle(cezaID, richardinuyesi.id, richardinuyesi.displayName, message.member.displayName, message.author.id, "TEMP-JAIL", sebep, zamanimiz, muteAtılma, muteBitiş, "jail")
let ceza = {  
...
let roller = richardinuyesi.roles.cache.filter(r => r.name !== "@everyone" && r.id !== registerConfig.booster).map(r => r.id)
/*db.set(`jroles.${richardinuyesi.id}`, roller)*/
if(richardinuyesi.voice.channel) richardinuyesi.voice.kick()
richardinuyesi.roles.cache.has(registerConfig.booster) ? richardinuyesi.roles.set([registerConfig.booster, moderationConfig.jailed]) : richardinuyesi.roles.set([moderationConfig.jailed]);
message.guild.log(cezaID, message.author, richardinuyesi, sebep, muteAtılma, muteBitiş, "jail", "jail-log")
//////////
setTimeout(() => {
    message.guild.unlog(cezaID, message.author, richardinuyesi, sebep, muteAtılma, muteBitiş, "jail", "jail-log")
    richardinuyesi.roles.remove(moderationConfig.jailed)
    richardinuyesi.roles.remove(moderationConfig.jailed)
