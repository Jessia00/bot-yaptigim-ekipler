...
module.exports.execute = async(ModerationClient , message, args) => {
    if (!message.member.roles.cache.has(moderationConfig.chatmute) && !message.member.hasPermission("ADMINISTRATOR")) return message.react(emojis.hata);
//////////
...
//////////
    ...
    let zamanimiz = zamanimizlar.replace("second"," saniye").replace("minute"," dakika").replace("hours"," saat").replace("day"," gün")
      ...
      moderationFunctions.embedBasari(message.author, message.channel, `${richardinuyesi} kullanıcısı yazılı kanallarda **${zamanimiz}** susturuldu (\`#${cezaID}\`)`)
    
//////////
moderationFunctions.cezaEkle(cezaID, richardinuyesi.id, richardinuyesi.displayName, message.member.displayName, message.author.id, "CHAT-MUTE", sebep, zamanimiz, muteAtılma, muteBitiş, "chatmute")
let ceza = {  
...
richardinuyesi.roles.add(moderationConfig.chatmuted)
message.guild.log(cezaID, message.author, richardinuyesi, sebep, muteAtılma, muteBitiş, "chatmute", "mute-log")
//////////
setTimeout(() => {
    message.guild.unlog(cezaID, message.author, richardinuyesi, sebep, muteAtılma, muteBitiş, "chatmute", "mute-log")
    richardinuyesi.roles.remove(moderationConfig.chatmuted)
