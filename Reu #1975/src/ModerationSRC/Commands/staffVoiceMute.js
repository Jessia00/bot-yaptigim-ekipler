...
module.exports.execute = async(ModerationClient , message, args) => {
    if (!message.member.roles.cache.has(moderationConfig.voicemute) && !message.member.hasPermission("ADMINISTRATOR")) return message.react(emojis.hata)
//////////
...
//////////
    ...
    let zamanimiz = zamanimizlar.replace("second"," saniye").replace("minute"," dakika").replace("hours"," saat").replace("day"," gün")
      ...
      moderationFunctions.embedBasari(message.author, message.channel, `${richardinuyesi} kullanıcısı sesli kanallarda **${zamanimiz}** susturuldu (\`#${cezaID}\`)`)
    
//////////
moderationFunctions.cezaEkle(cezaID, richardinuyesi.id, richardinuyesi.displayName, message.member.displayName, message.author.id, "VOICE-MUTE", sebep, zamanimiz, muteAtılma, muteBitiş, "voicemute")
let ceza = {  
...
richardinuyesi.roles.add(moderationConfig.voicemuted)
richardinuyesi.voice.setMute(true)
message.guild.log(cezaID, message.author, richardinuyesi, sebep, muteAtılma, muteBitiş, "voicemute", "mute-log")
//////////
setTimeout(() => {
    message.guild.unlog(cezaID, message.author, richardinuyesi, sebep, muteAtılma, muteBitiş, "voicemute", "mute-log")
    richardinuyesi.roles.remove(moderationConfig.voicemuted)
