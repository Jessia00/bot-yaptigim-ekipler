...
//////////
  ...
  if (!richardinuyesi || !sebep) return moderationFunctions.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${config.prefix || '.'}warn @Richârd/ID sebep\``).then(x => x.delete({timeout: 10000}));
...
moderationFunctions.cezaEkle(cezaID, richardinuyesi.id, richardinuyesi.displayName, message.member.displayName, message.author.id, "WARN", sebep, "KALICI", muteAtılma, "KALICI", "warn")
message.guild.log(cezaID, message.author, richardinuyesi, sebep, muteAtılma, "KALICI", "warn", "warn-log")
//////////
};
