...
if(!user) return moderationFunctions.embedHata(message.author, message.channel, 'Bir üye belirt.').sil(5)
if(!user.voice.channel) return moderationFunctions.embedHata(message.author, message.channel, 'bu kullanıcı seste değil').sil(5)
...
moderationFunctions.embedOlustur(message.author, message.channel, `${emojis.onay} Selam \`${message.member.displayName}\`, İstediğin  ${member} üyesinin ses bilgileri aşağıda belirtilmiştir.
\`\`\`diff
