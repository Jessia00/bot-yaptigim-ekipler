...
module.exports.execute = async(ModerationClient , message, args) => {
 let embed = new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setFooter(config.footer).setColor("RANDOM");
   ...
   let soncezainfo = cdb.get(`sonceza.${uyemiz.id}.${config.server}`);
     ...
     if (!sicilminerdelanit) return moderationFunctions.embedHata(message.author, message.channel, `**${message.guild.name}** sunucusunda kayıtlı ${uyemiz} kullanıcısının sicil kayıdı bulunmuyor.`).sil(5)
