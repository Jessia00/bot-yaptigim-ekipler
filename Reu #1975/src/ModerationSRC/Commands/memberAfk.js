...
module.exports.execute = async(ModerationClient, message, args) => {
  ...
  if(user.displayName.includes("[AFK]")){
   ...
   if(db.get(`${message.author.id}.afk`)) return moderationFunctions.embedHata(message.author, message.channel, `${emojis.hata} AFK durumdayken tekrardan __AFK__ olamazsın ${message.member}!`).then(x => x.delete({timeout: 5000}));;
