...
module.exports.execute = async(ModerationClient , message, args) => {
  ...
  if (!victim || message.author.id == victim.id) return moderationFunctions.embedHata(message.author, message.channel, `Bir üye belirtmelisin.`).then(x => x.delete({timeout: 10000}));
  if (!message.member.voice.channel || !victim.voice.channel || message.member.voice.channelID == victim.voice.channelID) return moderationFunctions.embedHata(message.author, message.channel, `Belirtilen üye veya sen sesli kanalda değilsin.`).then(x => x.delete({timeout: 10000}));
