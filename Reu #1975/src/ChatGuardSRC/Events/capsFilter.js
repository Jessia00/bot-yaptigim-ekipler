...
let muteBitiş = `${bitişGün} ${bitişAy.replace("01", "Ocak").replace("02", "Şubat").replace("03", "Mart").replace("04", "Nisan").replace("05", "Mayıs").replace("06", "Haziran").replace("07", "Temmuz").replace("08", "Ağustos").replace("09", "Eylül").replace("10", "Ekim").replace("11", "Kasım").replace("12", "Aralık")} ${bitişYıl} ${bitişSaat}`;
    await chatGuardFunctions.embedHata(message.author, message.channel, `${message.member} kişisi sunucumuzda caps kullanımı yasak olduğu halde 5 kere paylaştığı için \`5 dakika\` susturuldu! **[\`#${cezaID}\`]**`).then(x => x.delete({timeout: 10000}));  
//////////
moderationFunctions.botCezaEkle(cezaID, message.author.id, message.member.displayName, "CHAT-MUTE", "Caps Koruması", muteAtılma, muteBitiş, "chatmute")
...
komut: "CHAT-MUTE" 
    ...
    message.delete().then(async() => {
      ...
      await chatGuardFunctions.embedHata(message.author, message.channel, `${message.member}** Caps kullanımı yasaktır!**`).then(x => x.delete({timeout: 10000}));
  })
}
}; 
