...
/////////////// KÜFÜR ENGEL ///////////////
    ...
    let miktar = db.get(`kufur_${newMsg.author.id}`);
    if(miktar >= 5) {
        db.delete(`kufur_${newMsg.author.id}`)
let cezaID = cdb.get(`cezanumarasi.${newMsg.guild.id}`)+1;
let sahtezaman = "10m"
...
let muteBitiş = `${bitişGün} ${bitişAy.replace("01", "Ocak").replace("02", "Şubat").replace("03", "Mart").replace("04", "Nisan").replace("05", "Mayıs").replace("06", "Haziran").replace("07", "Temmuz").replace("08", "Ağustos").replace("09", "Eylül").replace("10", "Ekim").replace("11", "Kasım").replace("12", "Aralık")} ${bitişYıl} ${bitişSaat}`;
    await chatGuardFunctions.embedHata(newMsg.author, newMsg.channel, `${newMsg.member} kişisi sunucumuzda Küfür kullanımı yasak olduğu halde 5 kere paylaştığı için \`10 dakika\` susturuldu! **[\`#${cezaID}\`]**`).then(x => x.delete({timeout: 10000}));  
//////////
moderationFunctions.botCezaEkle(cezaID, newMsg.author.id, newMsg.member.displayName, "CHAT-MUTE", "Küfür Koruması", muteAtılma, muteBitiş, "chatmute")
newMsg.member.roles.add(moderationConfig.chatmuted)
//////////
setTimeout(() => {
newMsg.member.roles.remove(moderationConfig.chatmuted)
let cezalar = {  mod: "CHATGUARD", 
  sebep: "Küfür Koruması", 
  uye: newMsg.author.id,
  kisi: newMsg.member.displayName, 
  id: cezaID, 
  durum: `❌ [PASIF]`,
  zaman: muteAtılma,
  bitis: muteBitiş,
...
cdb.set(`sonceza.${newMsg.author.id}.${newMsg.guild.id}`, { 
mod: "CHATGUARD",
sebep: "Küfür Koruması", 
id: cezaID,  
durum: `❌ [PASIF]`,
...
komut: "CHAT-MUTE" 
    ...
    newMsg.delete().then(async() => {
      await db.add(`kufur_${newMsg.author.id}`, 1)
      await chatGuardFunctions.embedHata(newMsg.author, newMsg.channel, `${newMsg.member}** Küfür kullanımı yasaktır!**`).then(x => x.delete({timeout: 10000}));
  })
}
/////////////// KÜFÜR ENGEL ///////////////
...
if(kelimeler.some(a => newMsg.content.includes(a))){
  if(newMsg.guild.members.cache.get(newMsg.author.id).hasPermission('ADMINISTRATOR')) return;
  let miktar = db.get(`reklam_${newMsg.author.id}`);
  if(miktar >= 5) {
      db.delete(`reklam_${newMsg.author.id}`)
let cezaID = cdb.get(`cezanumarasi.${newMsg.guild.id}`)+1;
let sahtezaman = "10m"
...
let bitişYıl = moment(Date.now()+ms(sahtezaman)).format("YYYY");
let muteBitiş = `${bitişGün} ${bitişAy.replace("01", "Ocak").replace("02", "Şubat").replace("03", "Mart").replace("04", "Nisan").replace("05", "Mayıs").replace("06", "Haziran").replace("07", "Temmuz").replace("08", "Ağustos").replace("09", "Eylül").replace("10", "Ekim").replace("11", "Kasım").replace("12", "Aralık")} ${bitişYıl} ${bitişSaat}`;
  await chatGuardFunctions.embedHata(newMsg.author, newMsg.channel, `${newMsg.member} kişisi sunucumuzda link paylaşımı yasak olduğu halde 5 kere paylaştığı için \`10 dakika\` susturuldu! **[\`#${cezaID}\`]**`).then(x => x.delete({timeout: 10000}));  
//////////
moderationFunctions.botCezaEkle(cezaID, newMsg.author.id, newMsg.member.displayName, "CHAT-MUTE", "Reklam Koruması", muteAtılma, muteBitiş, "chatmute")
newMsg.member.roles.add(moderationConfig.chatmuted)
//////////
setTimeout(() => {
newMsg.member.roles.remove(moderationConfig.chatmuted)
...
komut: "CHAT-MUTE" 
  ...
  newMsg.delete().then(async() => {
    ...
    await chatGuardFunctions.embedHata(newMsg.author, newMsg.channel, `${newMsg.member}** link paylaşımı yasaktır!**`).then(x => x.delete({timeout: 10000}));
})
}
/////////////// REKLAM ENGEL ///////////////
...
/////////////// CAPS ENGEL ///////////////
    ...
    if (Math.round(yuzde) > 50) {
      if(newMsg.guild.members.cache.get(newMsg.author.id).hasPermission('ADMINISTRATOR')) return;
    let miktar = db.get(`caps_${newMsg.author.id}`);
    if(miktar >= 5) {
        db.delete(`caps_${newMsg.author.id}`)
let cezaID = cdb.get(`cezanumarasi.${newMsg.guild.id}`)+1;
let sahtezaman = "5m"
...
let bitişYıl = moment(Date.now()+ms(sahtezaman)).format("YYYY");
let muteBitiş = `${bitişGün} ${bitişAy.replace("01", "Ocak").replace("02", "Şubat").replace("03", "Mart").replace("04", "Nisan").replace("05", "Mayıs").replace("06", "Haziran").replace("07", "Temmuz").replace("08", "Ağustos").replace("09", "Eylül").replace("10", "Ekim").replace("11", "Kasım").replace("12", "Aralık")} ${bitişYıl} ${bitişSaat}`;
    await chatGuardFunctions.embedHata(newMsg.author, newMsg.channel, `${newMsg.member} kişisi sunucumuzda caps kullanımı yasak olduğu halde 5 kere paylaştığı için \`5 dakika\` susturuldu! **[\`#${cezaID}\`]**`).then(x => x.delete({timeout: 10000}));  
//////////
moderationFunctions.botCezaEkle(cezaID, newMsg.author.id, newMsg.member.displayName, "CHAT-MUTE", "Caps Koruması", muteAtılma, muteBitiş, "chatmute")
newMsg.member.roles.add(moderationConfig.chatmuted)
//////////
setTimeout(() => {
newMsg.member.roles.remove(moderationConfig.chatmuted)
...
komut: "CHAT-MUTE" 
    ...
    newMsg.delete().then(async() => {
      await db.add(`caps_${newMsg.author.id}`, 1)
      await chatGuardFunctions.embedHata(newMsg.author, newMsg.channel, `${newMsg.member}** Caps kullanımı yasaktır!**`).then(x => x.delete({timeout: 10000}));
  })
}
/////////////// CAPS ENGEL ///////////////
}; 
