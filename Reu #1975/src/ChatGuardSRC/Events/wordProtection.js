...
module.exports = async (message) => {
  ...
  if(message.author.bot) return;
    ...
    let miktar = db.get(`kufur_${message.author.id}`);
    if(miktar >= 5) {
        db.delete(`kufur_${message.author.id}`)
...
let muteBitiş = `${bitişGün} ${bitişAy.replace("01", "Ocak").replace("02", "Şubat").replace("03", "Mart").replace("04", "Nisan").replace("05", "Mayıs").replace("06", "Haziran").replace("07", "Temmuz").replace("08", "Ağustos").replace("09", "Eylül").replace("10", "Ekim").replace("11", "Kasım").replace("12", "Aralık")} ${bitişYıl} ${bitişSaat}`;
    await chatGuardFunctions.embedHata(message.author, message.channel, `${message.member} kişisi sunucumuzda Küfür kullanımı yasak olduğu halde 5 kere paylaştığı için \`10 dakika\` susturuldu! **[\`#${cezaID}\`]**`).then(x => x.delete({timeout: 10000}));  
//////////
moderationFunctions.botCezaEkle(cezaID, message.author.id, message.member.displayName, "CHAT-MUTE", "Küfür Koruması", muteAtılma, muteBitiş, "chatmute")
...
komut: "CHAT-MUTE" 
    ...
    message.delete().then(async() => {
      await db.add(`kufur_${message.author.id}`, 1)
      await chatGuardFunctions.embedHata(message.author, message.channel, `${message.member}** Küfür kullanımı yasaktır!**`).then(x => x.delete({timeout: 10000}));
  })
}
