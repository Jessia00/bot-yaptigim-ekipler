...
module.exports.execute = async(RegisterClient, message, args) => {
    ...
    let yazilacakisim;
    let isim = args[1];
    if(!member.user.username.includes(settings.tag) && !member.user.discriminator.includes("1998") && !member.roles.cache.has(registerConfig.vip) && !member.roles.cache.has(registerConfig.booster)) return message.react(emojis.hata);
