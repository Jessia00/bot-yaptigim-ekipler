...
module.exports.execute = async(RegisterClient, message, args) => {
    ...
    if (!member) return registerfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}isim @Richârd/ID <İsim> İsim\``).sil(10000);
    let yazilacakisim;
    let isim = args[1];
