    ...
    if(!message.guild || message.author.bot || message.content.toLowerCase().includes(`${settings.prefix}afk`)) return;
      if(message.mentions.users.size >= 1){
        ...
        if(memberdb.cek(`${victim.id}.afk`)) {
          let data = memberdb.cek(`${victim.id}.afk.sebep`);
        return message.channel.send(moderationfx.embedOlustur(message.author, message.channel,`:tada: ${victim} adlı üye **${data}** sebebi ile *AFK* modunda.`, "RANDOM")).then(x => x.delete({timeout: 7000}));;
        }
      };
   }; 
