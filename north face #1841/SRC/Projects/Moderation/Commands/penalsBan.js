...
if (!member) return moderationfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}ban @Richârd/ID Sebep\``).sil(10000);
if (!sebep) return moderationfx.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${settings.prefix || '.'}ban @Richârd/ID Sebep\``).sil(10000);
...
let cezaID = penals.cek(`cezanumarasi.${message.guild.id}`)+1;
      ...
      moderationfx.embedBasari(message.author, message.channel, `${member} kullanıcısı ${sebep} sebebi ile uzaklaştırıldı! (\`#${cezaID}\`)`)
...
moderationfx.cezaEkle(cezaID, member.id, member.displayName, message.member.displayName, message.author.id, "BAN", sebep, "KALICI", muteAtılma, "KALICI", "ban")
message.guild.log(cezaID, message.author, member, sebep, muteAtılma, "KALICI", "ban", "penal-log")
//////////
};
