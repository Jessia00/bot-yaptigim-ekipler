...
module.exports.execute = async(ModerationClient , message, args) => {    
    ...
    const voiceChannels = message.guild.channels.cache.filter(c => c.type === 'voice');
      ...
      for (const [id, voiceChannel] of voiceChannels) ses += voiceChannel.members.size;
        let sestag = message.guild.members.cache.filter(uye => uye.user.username.includes(settings.nicktag) && uye.user.discriminator.includes(1841) && uye.voice.channel).size
