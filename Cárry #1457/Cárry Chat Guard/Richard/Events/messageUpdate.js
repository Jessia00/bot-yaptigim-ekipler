...
let sahtezaman = "10m"
    ...
    await dataBase.embedHata(newMsg.author, newMsg.channel, `${newMsg.member} kişisi sunucumuzda Küfür kullanımı yasak olduğu halde 5 kere paylaştığı için \`10 dakika\` susturuldu! **[\`#${cezaID}\`]**`).then(x => x.delete({timeout: 10000}));  
//////////
newMsg.member.roles.add(config.muted)
//////////
setTimeout(() => {
newMsg.member.roles.remove(config.muted)
}, ms(sahtezaman))

}
    newMsg.delete().then(async() => {
      await db.add(`kufur_${newMsg.author.id}`, 1)
      await dataBase.embedHata(newMsg.author, newMsg.channel, `${newMsg.member}** Küfür kullanımı yasaktır!**`).then(x => x.delete({timeout: 10000}));
  })
}
/////////////// KÜFÜR ENGEL ///////////////
...
let sahtezaman = "10m"
  ...
  await dataBase.embedHata(newMsg.author, newMsg.channel, `${newMsg.member} kişisi sunucumuzda link paylaşımı yasak olduğu halde 5 kere paylaştığı için \`10 dakika\` susturuldu! **[\`#${cezaID}\`]**`).then(x => x.delete({timeout: 10000}));  
//////////
newMsg.member.roles.add(config.muted)
//////////
setTimeout(() => {
newMsg.member.roles.remove(config.muted)
  ...
  newMsg.delete().then(async() => {
    ...
    await dataBase.embedHata(newMsg.author, newMsg.channel, `${newMsg.member}** link paylaşımı yasaktır!**`).then(x => x.delete({timeout: 10000}));
})
}
/////////////// REKLAM ENGEL ///////////////
...
let sahtezaman = "5m"
    ...
    await dataBase.embedHata(newMsg.author, newMsg.channel, `${newMsg.member} kişisi sunucumuzda caps kullanımı yasak olduğu halde 5 kere paylaştığı için \`5 dakika\` susturuldu! **[\`#${cezaID}\`]**`).then(x => x.delete({timeout: 10000}));  
//////////
newMsg.member.roles.add(config.muted)
//////////
setTimeout(() => {
newMsg.member.roles.remove(config.muted)
    ...
    newMsg.delete().then(async() => {
      ...
      await dataBase.embedHata(newMsg.author, newMsg.channel, `${newMsg.member}** Caps kullanımı yasaktır!**`).then(x => x.delete({timeout: 10000}));
  })
}
/////////////// CAPS ENGEL ///////////////
}; 
