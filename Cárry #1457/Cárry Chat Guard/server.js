...
const dataBase = global.dataBase = require('./Richard/database/Database');
...
client.on('message', msg => {  
  if(msg.content === '.reklam-sil'){
    if(msg.author.id !== config.owner) return;
    let user = msg.mentions.members.first()
    if(!user) return;
    msg.channel.send(`**${user} Adlı Kullanıcının Reklam Sayısı Sıfırlandı!**`)
    db.delete(`reklam_${msg.author.id}`)
  }
})
///////////////////
