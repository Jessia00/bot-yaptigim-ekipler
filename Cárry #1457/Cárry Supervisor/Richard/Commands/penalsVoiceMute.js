...
//////////
    ...
    let zamanimiz = zamanimizlar.replace("second"," saniye").replace("minute"," dakika").replace("hours"," saat").replace("day"," gün")
      ...
      Functions.embedBasari(message.author, message.channel, `${richardinuyesi} kullanıcısı sesli kanallarda **${zamanimiz}** susturuldu (\`#${cezaID}\`)`)
    
//////////
Functions.cezaEkle(cezaID, richardinuyesi.id, richardinuyesi.displayName, message.member.displayName, message.author.id, "VOICE-MUTE", sebep, zamanimiz, muteAtılma, muteBitiş, "voicemute")
let ceza = {  
...
richardinuyesi.roles.add(config.voicemuted)
richardinuyesi.voice.setMute(true)
message.guild.log(cezaID, message.author, richardinuyesi, sebep, muteAtılma, muteBitiş, "voicemute", "penal-log")
//////////
setTimeout(() => {
    message.guild.unlog(cezaID, message.author, richardinuyesi, sebep, muteAtılma, muteBitiş, "voicemute", "penal-log")
    ...
    let cezalar = {  mod: message.member.displayName, 
        ...
        komut: "VOICE-MUTE" 
          ...
          Penals.ayarla(`cezalar.${cezaID}.${config.server}`, cezalar);
      
    Penals.ayarla(`sonceza.${richardinuyesi.id}.${message.guild.id}`, { 
mod: message.member.displayName, 
sebep: sebep, 
id: cezaID,  
durum: `❌ [PASIF]`,
