...
//////////
  ...
  if (!richardinuyesi || !sebep || !zaman) return Functions.embedHata(message.author, message.channel, `Komutu doğru kullanmalısın. \`Örnek: ${config.prefix || '.'}unchatmute @Richârd/ID (1s/1m/1h/1d) sebep\``).then(x => x.delete({timeout: 10000}));
...
//////////
      ...
      Functions.embedBasari(message.author, message.channel, `${richardinuyesi} kullanıcısı yazılı kanallardaki susturulması açıldı`)
