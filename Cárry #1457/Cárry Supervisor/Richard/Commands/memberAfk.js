...
module.exports.execute = async(client, message, args) => {
  ...
  if(user.displayName.includes("[AFK]")){
   ...
   if(User.cek(`${message.author.id}.afk`)) return Functions.embedHata(message.author, message.channel, `${emojis.hata} AFK durumdayken tekrardan __AFK__ olamazsın ${message.member}!`).then(x => x.delete({timeout: 5000}));;
  ...
  if(user.manageable) user.setNickname('[AFK] ' + user.displayName)
       User.ayarla(`${message.author.id}.afk`, true)
       User.ayarla(`${message.author.id}.afk.sure`, new Date());
