...
const User = global.User = new Veritabani("./Richard/Models/User");
const Penals = global.Penals = new Veritabani("./Richard/Models/Penals");
const Classic = global.Classic = new Veritabani("./Richard/Models/Classic");
const Staff = global.Staff = new Veritabani("./Richard/Models/Staff");
const Premium = global.Premium = new Veritabani("./Richard/Models/Premium");
...
client.on('guildMemberRoleAdd', async(member, role) =>  {
  ...
  let tarihxd = `${atilanGün} ${atilanAy.replace("01", "Ocak").replace("02", "Şubat").replace("03", "Mart").replace("04", "Nisan").replace("05", "Mayıs").replace("06", "Haziran").replace("07", "Temmuz").replace("08", "Ağustos").replace("09", "Eylül").replace("10", "Ekim").replace("11", "Kasım").replace("12", "Aralık")} ${atilanYıl} ${atilanSaat}`;
      ...
      User.ekle(`rolsayi_${member.id}`, +1)
      User.degerekle(`rollogu.${member.id}_qwe`, {
      verilenrol: role.id,
      emoji: emojis.onay,
      roldurum: "verildi",
